...
import { k8sApi, type K8sConfigMap, type K8sCluster } from '../../../api/k8s'
...
const columns = [
  ...
  { title: 'Data 数量', dataIndex: 'dataCount' },
...
async function fetchList() {
  ...
  loading.value = true
  try {
    const res = await k8sApi.listConfigMaps({ clusterId: selectedClusterId.value!, namespace: namespace.value || undefined })
    list.value = res || []
  } finally {
    loading.value = false
  }
...
<template>
  <a-card>
    <div style="margin-bottom: 16px; display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
      <h3 style="margin: 0;">ConfigMap 列表</h3>
